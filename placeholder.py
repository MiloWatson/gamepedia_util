        ...
        if template2.name.matches('TournamentResultsLine'):
            ...
            while template2.has('p' + str(i)):
                if template2.has('p' + str(i) + 'link'):
                    p["player{0}".format(i)] = template2.get('p' + str(i) + 'link').value.strip()
                    template2.remove('p' + str(i))
                    template2.remove('p' + str(i) + 'link')
                else:
                    p["player{0}".format(i)] = template2.get('p' + str(i)).value.strip()
                    template2.remove('p' + str(i))
                if template2.has('p' + str(i) + 'role'):
                    r["role{0}".format(i)] = template2.get('p' + str(i) + 'role').value.strip()
...
for page in pages:
    ...
    #         process_line()
    process_page()
    re.sub(regex, '', str(wikitext))
    re.sub(r'{{TournamentResultEnd}}', '}}', str(wikitext))
